...
//				}
	...
	class BtServerThread extends Thread {
		...
		public void run() {
			...
			try {
				if (mSecure) {
					bluetoothServerSocket = mBluetoothAdapter.listenUsingRfcommWithServiceRecord(NAME_SECURE, AppConfig.UUID_SECURE);
				} else {
					bluetoothServerSocket = mBluetoothAdapter.listenUsingInsecureRfcommWithServiceRecord(NAME_INSECURE, AppConfig.UUID_INSECURE);
				}
			} catch (IOException e) {
