...
public class OtaManager {
	...
	private void retryUpdate(BleOtaUpdater otaUpdater) {
		...
		startOtaUpdate(mOtaFile, otaUpdater.getBleDevice());
	}
	 ...
	 *
	 * @param file      更新文件
	 * @param bleDevice 更新设备
	 * @return 是否正确执行更新
	 */
	public boolean startOtaUpdate(File file, BleDevice bleDevice) {
		if (bleDevice == null || file == null || !file.exists() || !file.canRead()) {
			return false;
		}
		...
		try {
			...
			mStopUpdate = false;
			if (mBleOtaUpdater.otaStart(mFilePath, bleDevice) == BleGlobalVariables.OtaResult.OTA_RESULT_SUCCESS) {
				return true;
			}
		} catch (IOException e) {
			if (AppConfig.DEBUG) {
