...
public class BleOtaUpdater implements BleDevice.OtaListener {
	...
	private BleGlobalVariables.OtaResult mRetValue;//返回值
	...
	@Override
	public void onChange(BleDevice device, byte[] data) {
		otaGetResult(data);
	}
	...
	private BleGlobalVariables.OtaCmd valueToCmd(int val) {
		switch (val & 255) {
			case 1:
	...
	private boolean otaWrite(byte[] data) throws InterruptedException {
		...
		if (this.shouldStopUpdate()) {
			...
			return false;
		} else if (!this.mBleDevice.writeOtaData(data)) {//是否写入失败
			if(AppConfig.DEBUG) {
	 ...
	 * @return 是否发送成功
	 */
	private boolean otaSendPacket(BleGlobalVariables.OtaCmd cmd, short checksum, byte[] data, int dataLength) {
		// 获取命令字节
	...
	private int otaSendMetaData(FileInputStream fin) throws IOException {
		...
		if (readLength < 0) {
			...
			// 获取Meta的每字节的效验大小
			short checksum = this.cmdToValue(BleGlobalVariables.OtaCmd.OTA_CMD_META_DATA);

			// 统计整个meta的效验大小
			for (int i = 0; i < readLength; ++i) {
				checksum = (short) (checksum + (data[i] & 255));
			}
			return this.otaSendPacket(BleGlobalVariables.OtaCmd.OTA_CMD_META_DATA, checksum, data, dataLength) ? readLength + 2 : -1;
		}
	}
	...
	private int otaSendBrickData(FileInputStream fin, int dataLength) throws IOException {
		...
		if (readLength <= 0) {// 读取失败
			...
			// 获取每字节效验值大小
			short checksum = this.cmdToValue(BleGlobalVariables.OtaCmd.OTA_CMD_BRICK_DATA);
			...
			// 发送数据包
			if (this.otaSendPacket(BleGlobalVariables.OtaCmd.OTA_CMD_BRICK_DATA, checksum, data, dataLength)) {
				return readLength;
			} else {
				if(AppConfig.DEBUG) {
	...
	private boolean otaSendVerifyCmd() {
		byte checksum = this.cmdToValue(BleGlobalVariables.OtaCmd.OTA_CMD_DATA_VERIFY);
		return this.otaSendPacket(BleGlobalVariables.OtaCmd.OTA_CMD_DATA_VERIFY, checksum, null, 0) && this.waitVerifyCmdDone();
	}
	...
	private void otaSendResetCmd() {
		byte checksum = this.cmdToValue(BleGlobalVariables.OtaCmd.OTA_CMD_EXECUTION_NEW_CODE);
		this.otaSendPacket(BleGlobalVariables.OtaCmd.OTA_CMD_EXECUTION_NEW_CODE, checksum, null, 0);
	}
	...
	public void otaGetResult(byte[] notify_data) {
		BleGlobalVariables.OtaCmd cmdType = this.valueToCmd(notify_data[2] & 255);
		if (cmdType == null) {
			this.otaPrintBytes(notify_data, "Notify data: ");
			this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_RECEIVED_INVALID_PACKET);
		} else {
			switch (notify_data[3]) {
				case 0:
					...
					break;
				case 1:
					this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_PKT_CHECKSUM_ERROR);
					break;
				case 2:
					this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_PKT_LEN_ERROR);
					break;
				case 3:
					this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_DEVICE_NOT_SUPPORT_OTA);
					break;
				case 4:
	 ...
	 * @return 更新结果
	 */
	public BleGlobalVariables.OtaResult otaStart(String file, BleDevice bleDevice) {
		if (!file.isEmpty() && bleDevice != null) {
			this.mFilePath = file;
			this.mBleDevice = bleDevice;
			this.mBleDevice.setOtaListener(this);
	...
	private void otaUpdateProcess(String filePath) {
		try {
			...
			if (offset1 < 0 || mShouldStop) {// 偏移量有误
				...
				fileInputStream.close();
				this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_META_RESPONSE_TIMEOUT);
				if (mHandler != null) {
					mHandler.obtainMessage(OTA_FAIL, mIndex).sendToTarget();
				}
				return;
			...
			do {// 遍历读取文件 每次发送 mPacketSize 个字节
				...
				if (ret1 < 0 || mShouldStop) {
					...
					this.serErrorCode(BleGlobalVariables.OtaResult.OTA_RESULT_DATA_RESPONSE_TIMEOUT);
					if (mHandler != null) {
						mHandler.obtainMessage(OTA_FAIL, mIndex).sendToTarget();
					}
					return;
